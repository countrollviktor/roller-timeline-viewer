  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-2 grid grid-cols-3 items-center gap-3">
          <a href="/" className="justify-self-start">
            <img src="/countroll-logo.svg" alt="Countroll" className="h-5" />
          </a>
          <form onSubmit={handleSearch} className="flex gap-1 justify-self-center">
            <input
              type="text"
              value={searchId}
              onChange={e => setSearchId(e.target.value)}
              placeholder="Go to asset..."
              className="w-32 sm:w-48 px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#1DB898]"
            />
            <button
              type="submit"
          ...
          <div className="flex items-center gap-3 justify-self-end">
            {getStatsAccess() && (
              <Link
                ...
                title="Stats"
                aria-label="Stats"
                className="text-gray-500 hover:text-[#1DB898] p-1 rounded"
              >
                <svg
